...
struct AddSubscriptionView: View {
            ...
            VStack(spacing: 0) {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 20) {
                        ...
                        // Сумма + валюта + период + дата начала
                        paymentSection
                ...
                // Кнопка прижата к низу, скролл знает её высоту и не уходит под неё
            ...
            FormField(label: "Валюта", isRequired: true) {
                HStack(spacing: 8) {
                    ForEach(Currency.allCases, id: \.self) { cur in
                        CurrencyChip(
                            currency: cur,
                            isSelected: viewModel.currency == cur
                        ) {
            ...
            FormField(label: "Периодичность", isRequired: true) {
                HStack(spacing: 8) {
                    ForEach(BillingPeriod.allCases, id: \.self) { period in
                        PeriodChip(
                            period: period,
                            isSelected: viewModel.billingPeriod == period
                        ) {
    ...
    private var categorySection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                ...
                .padding(.horizontal, 1) // чтобы не обрезало тень
            }
        }
    }
                ...
                FormField(label: "Введите тег", isRequired: false) {
                    TextField("Личное, Бизнес…", text: $viewModel.tagInput)
                        ...
                        .onChange(of: viewModel.tagInput) { _, _ in
                            // Сбрасываем выбранный тег при ручном вводе
    ...
    private var saveButton: some View {
        Button(action: {
            Task {
                await viewModel.save { subscription in
